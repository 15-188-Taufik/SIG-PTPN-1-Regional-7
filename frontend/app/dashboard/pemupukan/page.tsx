...
export default function PemupukanPage() {
  ...
  // State
  const [data, setData] = useState<PemupukanListResponse>({
    total: 0,
    total_luas: 0,
    total_jumlah_pupuk: 0,
    total_hk: 0,
    items: [],
  });
  ...
  const [editingItem, setEditingItem] = useState<PemupukanItem | null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.detail || 'Gagal memuat data pemupukan harian');
    } finally {
      setLoading(false);
    }
  ...
  function handleOpenEdit(item: PemupukanItem) {
    setEditingItem(item);
    setIsModalOpen(true);
  }

  async function handleDelete(id: number) {
    if (!confirm('Apakah Anda yakin ingin menghapus catatan pemupukan ini?')) return;
    try {
      await deletePemupukan(id);
      loadData();
    } catch (err: any) {
  ...
  async function handleFormSubmit(formData: any) {
    if (editingItem) {
      await updatePemupukan(editingItem.id, formData);
    } else {
      await createPemupukan(formData);
    }
    loadData();
  }
  ...
  function handleExportExcel() {
    try {
      const rows = data.items.map((row, idx) => ({
        ...
        'Jenis Pupuk': row.jenis_pupuk || '-',
        'Jumlah Pupuk (Kg)': row.jumlah_pupuk != null ? row.jumlah_pupuk : 0,
      ...
      utils.book_append_sheet(wb, ws, 'Pemupukan Harian');
      
      const dateStr = new Date().toISOString().slice(0, 10);
      writeFile(wb, `Laporan_Pemupukan_Harian_${dateStr}.xlsx`);
    } catch (err) {
      ...
      <main style={styles.mainContent}>
        ...
        <div style={styles.kpiGrid}>
          <div style={styles.kpiCard}>
            <div style={styles.kpiLabel}>Total Transaksi Pemupukan</div>
            <div style={styles.kpiValue}>{data.total.toLocaleString('id-ID')}</div>
            <div style={styles.kpiSub}>Catatan aplikasi pupuk</div>
          </div>
          <div style={styles.kpiCard}>
            <div style={styles.kpiLabel}>Total Luas Aplikasian</div>
            ...
            <div style={styles.kpiSub}>Total area terpupuk</div>
          </div>
          <div style={styles.kpiCard}>
            <div style={styles.kpiLabel}>Total Jumlah Pupuk</div>
            <div style={{ ...styles.kpiValue, color: '#f5a623' }}>
              {data.total_jumlah_pupuk.toLocaleString('id-ID', { minimumFractionDigits: 1 })}{' '}
              <span style={{ fontSize: '16px' }}>Kg</span>
            </div>
            <div style={styles.kpiSub}>Total pupuk teraplikasi</div>
          </div>
          <div style={styles.kpiCard}>
        ...
        <div style={styles.filterCard}>
          <div style={styles.filterGrid}>
            ...
            <div style={{ ...styles.filterGroup, flex: 2 }}>
              <label style={styles.filterLabel}>Pencarian (Jenis Pupuk / Kode Blok)</label>
              <input
                type="text"
                placeholder="Cari jenis pupuk (NPK, Urea, dll)..."
                value={search}
        ...
        <div style={styles.tableCard}>
          <div style={styles.tableHeaderBar}>
            <div style={{ fontSize: '13px', fontWeight: '600', color: '#161616' }}>
              Daftar Catatan Pemupukan ({data.items.length} catatan terurai)
            </div>
          </div>
            ...
            <table style={styles.table}>
              <thead>
                <tr style={styles.thRow}>
                  ...
                  <th style={styles.th}>Jenis Pupuk</th>
                  <th style={styles.th}>Jumlah Pupuk (Kg)</th>
                  ...
                  paginatedItems.map((row, idx) => {
                    ...
                    return (
                      <tr key={row.id} style={idx % 2 === 1 ? styles.trEven : styles.trOdd}>
                        ...
                        <td style={{ ...styles.td, fontWeight: '600', color: '#24a148' }}>{row.jenis_pupuk}</td>
                        <td style={{ ...styles.td, fontWeight: '700' }}>{row.jumlah_pupuk.toLocaleString('id-ID')} Kg</td>
