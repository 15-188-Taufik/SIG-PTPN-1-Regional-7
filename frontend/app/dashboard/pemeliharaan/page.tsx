...
export default function PemeliharaanPage() {
  ...
  function handleExportPDF() {
    try {
      ...
      doc.text('LAPORAN HARIAN PEMELIHARAAN KEBUN', 14, 21);
      ...
      doc.text('Rangkuman KPI Pemeliharaan:', 14, 37);

      autoTable(doc, {
        startY: 40,
        head: [['Total Kegiatan', 'Total Luas Aplikasi', 'Total Dosis Aplikasi', 'Total Tenaga Kerja']],
        body: [[
          `${data.total.toLocaleString('id-ID')} Catatan`,
          `${data.total_luas.toLocaleString('id-ID', { minimumFractionDigits: 1 })} Ha`,
          `${data.total_dosis.toLocaleString('id-ID', { minimumFractionDigits: 1 })} L/Kg`,
      ...
      doc.text('Daftar Rincian Pemeliharaan Harian:', 14, finalY);

      const tableHeaders = [
        ['No', 'Tanggal', 'Kebun / Unit', 'Afdeling', 'Kode Blok', 'Jenis Kegiatan', 'Bahan / Material', 'Dosis (L/Kg)', 'Luas (Ha)', 'Tenaga (HK)', 'Keterangan']
      ];
      ...
      const tableRows = data.items.map((row, idx) => [
        ...
        row.jenis_kegiatan || '-',
        row.material || '-',
        row.dosis_aplikasi != null ? row.dosis_aplikasi.toLocaleString('id-ID', { minimumFractionDigits: 1 }) : '-',
      ...
      doc.save(`Laporan_Pemeliharaan_Harian_${dateStr}.pdf`);
